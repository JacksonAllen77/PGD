...
public class AudioTest {
    ...
    public static void main(String args[]) throws InterruptedException {
        ...
        AudioTest.login_V40("10.19.36.113", (short) 8000, "admin", "abcd1234");
...
//            FileOutputStream Encodefile = null;
    ...
    public static void startTransPCMData(int luserID) {
        ...
        lVoiceHandle = hCNetSDK.NET_DVR_StartVoiceCom_MR_V30(lUserID, 1, cbVoicePcmDataCallBack_mr_v30, null);
        if (lVoiceHandle < 0) {
        ...
        try {
            ...
            for (int i = 0; i < PCMdataLength / 1920; i++) {
                HCNetSDK.BYTE_ARRAY ptrPcmSend = new HCNetSDK.BYTE_ARRAY(1920);
                System.arraycopy(ptrVoicePcmByte.byValue, i * 1920, ptrPcmSend.byValue, 0, 1920);
                ptrPcmSend.write();
                if ( hCNetSDK.NET_DVR_VoiceComSendData(lVoiceHandle, ptrPcmSend.byValue, 1920) ==false) {
                    System.out.println("NET_DVR_VoiceComSendData failed, error code:" + hCNetSDK.NET_DVR_GetLastError());
                }
                else
                {
                    System.out.println("NET_DVR_VoiceComSendData 1920 succeed ,data is " + i * 1920);
                }
                //需要实时速率发送数据
                try {
                    Thread.sleep(40);
